    ...
    def __init__(self, y, X, standardize = True):
        ...
        with self.model:
            ...
            for i in range(self.num_of_imputation):
                self.weight_a0.append(pm3.Normal("alpha"+str(i), mu = 0, sigma = self.a0))  
                self.weight.append(pm3.Normal("beta"+str(i), mu = 0, sigma = self.ai, shape = self.p))

            # set likelihood
            self.y_obs = []
            for j in range(self.num_of_imputation):
                self.y_obs.append(pm3.Normal("y_obs" + str(j), mu = self.weight_a0[j] + pm3.math.dot(self.X[j], self.weight[j]), sigma = self.sigma2, observed = self.y[j]))
