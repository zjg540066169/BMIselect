...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 26 09:11:00 2022

This class provides the model of Multi-Laplace.

Two hyperparameters: r, s.
...
class Laplace(Bmi_lasso):
    def __init__(self, y, X, standardize = True, r = 2, s = 15):
        ...
        with self.model:
            ...
            self.rho = pm3.Gamma('rho', alpha = self.r, beta = self.s)
            self.lambda2 = pm3.Gamma("lambda2", alpha = (self.num_of_imputation + 1) / 2, beta = 2/(self.num_of_imputation * self.rho), shape = self.p)
            self.lambda2_0 = pm3.Gamma("lambda0", alpha = (self.num_of_imputation + 1) / 2, beta = 2/(self.num_of_imputation * self.rho))
            
            # set regression variables
            self.weight = []
            self.weight_a0 = []
            for i in range(self.num_of_imputation):
                self.weight_a0.append(pm3.Normal("alpha"+str(i), mu = 0, sigma = tt.sqrt(self.lambda2_0)))
                self.weight.append(pm3.Normal("beta"+str(i), mu = 0, sigma = tt.sqrt(self.lambda2), shape = self.p))
