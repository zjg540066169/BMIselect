...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep 23 14:24:00 2022

This class provides the model of Horseshoe.
...
class Horseshoe(Bmi_lasso):
    def __init__(self, y, X, standardize = True):
        ...
        with self.model:
            ...
            self.tau = pm3.HalfCauchy("tau", beta = 1)
            self.lambda_ = pm3.HalfCauchy("lambda", beta = 1, shape = self.p)
            self.lambda_0 = pm3.HalfCauchy("lambda0", beta = 1)
            
            # set regression variables
            self.weight = []
            self.weight_a0 = []
            for i in range(self.num_of_imputation):
                self.weight_a0.append(pm3.Normal("alpha"+str(i), mu = 0, sigma = tt.sqrt(tt.pow(self.lambda_0, 2) * tt.pow(self.tau, 2))))
                self.weight.append(pm3.Normal("beta"+str(i), mu = 0, sigma = tt.sqrt(tt.pow(self.lambda_, 2) * tt.pow(self.tau, 2)), shape = self.p))
...
if __name__ == "__main__":
    ...
    X_array = gendata["M_X"]
    Y_array = gendata["M_Y"]
    model = Horseshoe(Y_array, X_array)
